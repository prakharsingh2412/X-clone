...
export const deletePost= async (req,res) => {
    try {
        ...
        if (!post) return res.status(404).json({error: "post is not present"});
        if (post.user.toString() !== req.user._id.toString() ) {
            return res.status(401).json({message: "You are unautherised to delete the post"});
        }
        if (post.img){
...
export const commentOnPost = async (req,res) =>{
    try {
        ...
        if (!text) return res.status(400).json({error:'text field is empty'});
